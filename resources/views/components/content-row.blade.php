...
$details = Http::asJson()->get(config('services.tmdb.endpoint').'movie/' . $content .'?append_to_response=release_dates&api_key='.config('services.tmdb.api')) ->json();
...
<div class="grid grid-cols-6 mx-40 text-white text-opacity-50">
    <div class="col-span-5 grid grid-cols-6 items-center">
        ...
        <p>{{Carbon::parse($details['release_date'])->year}}</p>
        <p>{{ $runtime }}</p>
    </div>
